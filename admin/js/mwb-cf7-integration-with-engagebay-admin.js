...
jQuery( document ).ready( function() {
	...
	const enableDataTable = () => {
		...
		jQuery( '#mwb-' + crm + '-cf7-table' ).DataTable( {
         ...
         order: [ 1, 'asc' ],
			...
			language    : {
				"lengthMenu": mwb_cf7_integration_ajax_data.rows_per_page + " _MENU_",
				"info"      : "",
			}
		} );
	}
	...
	const triggerSuccess = ( msg = '' ) => {
		Swal.fire( {
			icon : 'success',
			title: mwb_cf7_integration_ajax_data.popup_process_title,
			text : msg,
			confirmButtonText: mwb_cf7_integration_ajax_data.sweetalert_button_text,
			timer: 1500,
		} );
	}
	 ...
	 * @since      1.0.0
	 */
	const triggerError = ( msg = mwb_cf7_integration_ajax_data.popup_error_msg ) => {
		Swal.fire( {
			icon : 'error',
			title: mwb_cf7_integration_ajax_data.popup_error_title,
			text :  msg,
			confirmButtonText: mwb_cf7_integration_ajax_data.sweetalert_button_text,
		} );
	}
	...
	const triggerInfo = ( msg = '' ) => {
		Swal.fire( {
			icon  : 'info',
			title : mwb_cf7_integration_ajax_data.alert_title,
			text  : msg,
			confirmButtonText: mwb_cf7_integration_ajax_data.sweetalert_button_text,
		} );
	}
	 ...
	 * @return  void
	 */
	 const triggerSetupDocs = ( crm, content ) => {
        uri = jQuery( '#mwb-'+ crm + '-redirect-uri' ).val();
        Swal.fire({
            title: mwb_cf7_integration_ajax_data.get_api_title,
            confirmButtonText: mwb_cf7_integration_ajax_data.sweetalert_button_text,
            html:content,
            allowOutsideClick: !1,
            showClass: {
	 ...
	 */
	 const createOrFilteredHtml = ( index = 1 ) => {
		...
		html += '<option value="-1">' + mwb_cf7_integration_ajax_data.select_field_filter + '</option>';
		...
		html += '<option value="-1">' + mwb_cf7_integration_ajax_data.select_condition_filter + '</option>';
	...
	jQuery( 'input[name="mwb_setting[delete_logs]"]' ).on( 'keyup change focusout', function() {
		...
		if ( jQuery( this ).val() % 1 != 0 ) {
			...
			if ( length > 4 ) {
				triggerError( mwb_cf7_integration_ajax_data.integer_value );
			}
		}
	} );
	...
	jQuery( document ).on( 'click', '.trigger-setup-guide', function() {
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				if ( true == response.data.success ) {
                    triggerSetupDocs( crm, response.data.content );
				} else {
					triggerError( response.data.message );
				}
			} else {
	...
	jQuery( '#mwb-' + crm + '-cf7-authorize-button' ).on( 'click', function( e ) {
		...
		let api_key = jQuery( '#mwb-' + crm + '-cf7-api-key' ).val();
		let base_url = jQuery( '#mwb-' + crm + '-cf7-base-url' ).val();
		
		if ( api_key.length == 0 || base_url.length == 0 ) {
			triggerError( mwb_cf7_integration_ajax_data.required_fields );
			return;
		} else {
	...
	jQuery( 'input[name="post_title"]' ).on( 'keyup', function() {
		...
		let feed_title = jQuery( 'input[name="post_title"]' ).val();
        if ( ! feed_title ) {
            let msg = '<span class="title_warning">' + mwb_cf7_integration_ajax_data.feed_title_empty + '</span>';
	...
	jQuery( '#mwb-feeds-' + crm + '-object' ).on( 'change', function() {
		...
		if ( '-1' == jQuery( '#mwb-' + crm + '-cf7-select-form' ).val() ) {
			...
			triggerInfo( mwb_cf7_integration_ajax_data.select_form );
			return;
		}
	...
	jQuery( document ).on( 'click' , '#add-new-field-btn', function( e ) {
		...
		if( '-1' == object || '-1' == field ) {
			triggerInfo( mwb_cf7_integration_ajax_data.valid_object_field );
			return;
		}
	...
	jQuery( document ).on( 'click', '.condition-or-btn', function( e ) {
		...
		let form = jQuery( '#mwb-' + crm + '-cf7-select-form' ).val();
		if ( '-1' == form ) {
			triggerInfo( mwb_cf7_integration_ajax_data.filter_valid_form );
			return;
		}
	...
	jQuery( document ).on( 'change', '.condition-option-field', function() {
		...
		if ( 'empty' == condition || 'not_empty' == condition ) {
			...
			value.val( '' ).removeAttr( 'disabled' ).attr( 'placeholder', mwb_cf7_integration_ajax_data.condition_filter_value );
		}
	} );
	...
	jQuery( '.mwb-feed-status' ).on( 'change', function() {
		...
		if ( this.checked ) {	
			...
			result.then( (response ) => {
				if ( true == response.status ) {
					jQuery( '.mwb-feed-status-text_' + feedId ).html( '<strong>' + mwb_cf7_integration_ajax_data.feed_active_text + '</strong>' );
				}
			} );
			...
			result.then( (response ) => {
				if ( true == response.status ) {
					jQuery( '.mwb-feed-status-text_' + feedId ).html( '<strong>' + mwb_cf7_integration_ajax_data.sandbox_text + '</strong>' );
				}
			} );
		}
	} );
	...
	jQuery( document ).on( 'change', '.filter-feeds-by-form', function() {
		...
		if ( '-1' == form_id ) {
			triggerInfo( mwb_cf7_integration_ajax_data.valid_form );
			return;
		}
	...
	jQuery( '#mwb-' + crm + '-cf7-clear-log' ).on( 'click', function( e ) {
		...
		button.text( mwb_cf7_integration_ajax_data.ajax_loading );
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				if ( true == response.data.success ) {
					button.text( mwb_cf7_integration_ajax_data.clear_log );
	...
	jQuery( '#mwb-' + crm + '-cf7-download-log' ).on( 'click', function( e ) {
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				button.text( mwb_cf7_integration_ajax_data.download_text );
				if ( true == response.data.success ) {
